...
reais (`id_pessoa`, `id_projeto`) e, se a política mandar, de nomes de pessoa.

**Política atual (2026-09-18, decisão do usuário — transparência total): nomes
de pessoas SÃO públicos.** Os dados vêm da Plataforma Sucupira, que os publica, e
o site mostra autores de produções e membros de projetos. Por isso a varredura de
nomes está desligada por padrão (`NOMES_SAO_PUBLICOS`); `--proibir-nomes` a religa.
O que continua proibido, e continua varrido, são os **identificadores reais**
(`id_pessoa`, `id_projeto`): o site usa ids substitutos e nunca precisou dos reais.
...
# Política: nome de pessoa em artefato público é permitido (ver docstring do
# módulo). Trocar para False religa a varredura de nomes — o código não foi
# removido justamente para a decisão poder ser revertida numa linha.
NOMES_SAO_PUBLICOS = True
...
def autoteste() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Com a varredura de nomes desligada (rx_nomes=None), nome não acusa —
        # mas id real continua acusando. É o que a política de 2026-09-18 pede.
        sem_nomes = [
            ("nome com varredura de nomes desligada", "m.json",
             '{"autor": "FULANA DE TAL SOBRENOME"}', False),
            ("id real com varredura de nomes desligada", "n.json",
             '{"id_pessoa": 4231, "autor": "FULANA DE TAL SOBRENOME"}', True),
        ]
        for descricao, nome, conteudo, deve in sem_nomes:
            f = base / nome
            f.write_text(conteudo, encoding="utf-8")
            achou = bool(varrer_json(f, pessoa_ids, projeto_ids, None))
            ok = achou == deve
            if not ok:
                falhas += 1
            print(f"  {'✓' if ok else '✗'} {descricao} ({'deve acusar' if deve else 'não deve acusar'})")

    if falhas:
        print(f"\n{falhas} caso(s) falharam — o guard não está confiável.")
        return 1
    print(f"\nOK — {len(casos) + 2} casos, detecção e ausência de falso positivo confirmadas.")
    return 0
...
def main() -> None:
    ...
    p.add_argument(
        "--proibir-nomes",
        action="store_true",
        help="religa a varredura de nomes de pessoa (desligada pela política de "
        "transparência de 2026-09-18)",
    )
    p.add_argument(
        "--autoteste",
        action="store_true",
    ...
    varrer_nomes = args.proibir_nomes or not NOMES_SAO_PUBLICOS
    rx_nomes = compilar_regex_nomes(nomes) if varrer_nomes else None
    print("  Nomes de pessoas: "
          + ("PROIBIDOS (varredura ligada)." if varrer_nomes
             else "públicos por política (transparência total, 2026-09-18) — não varridos."))
    ...
    if not violacoes:
        print("OK — nenhum identificador real nos artefatos"
              + (" (nem nome de pessoa)." if varrer_nomes else "; nomes são públicos por política."))
        sys.exit(0)
