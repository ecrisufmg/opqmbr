...
export default function Cartograma({
  ...
  /** Clique (ou Enter) numa ligação entre dois programas. */
  ...
  /** O que a ligação conta, com o aviso de que o recorte por tipo não acompanha o link. */
  function abrirLigacao(e: { a: string; b: string; n: number }, x: number, y: number) {
    if (!onLigacao) return;
    const todosTipos = categoriasAtivas.size === rede.categorias.length;
    onLigacao({
      a: e.a,
      b: e.b,
      resumo: `${e.n.toLocaleString("pt-BR")} produções em coautoria com pessoas do outro programa${todosTipos ? "" : " (só nos tipos selecionados)"}.`,
      aviso: todosTipos ? undefined : "O mapa mostra todos os tipos: o recorte por tipo do cartograma não é levado.",
      x,
      y,
    });
  }
    ...
    <div>
              ...
              return (
                <g key={chave} className="carto-aresta">
                  ...
                  <path
                    ...
                    role={onLigacao ? "button" : undefined}
                    tabIndex={onLigacao ? 0 : undefined}
                    aria-label={onLigacao ? `Ligação ${e.a} e ${e.b}: ${e.n} produções. Abrir opções` : undefined}
                    onClick={(ev) => abrirLigacao(e, ev.clientX, ev.clientY)}
                    onKeyDown={(ev) => {
                      if (ev.key !== "Enter" && ev.key !== " ") return;
                      ev.preventDefault();
                      const r = (ev.currentTarget as Element).getBoundingClientRect();
                      abrirLigacao(e, r.left + r.width / 2, r.top + r.height / 2);
                    }}
                  >
                    <title>
                      {`${e.a} × ${e.b}: ${e.n} produções${e.parciais > 0 ? ` (+${e.parciais} tipo(s) com contagem suprimida)` : ""}`}
                    </title>
                  </path>
                </g>
              );
              ...
              return (
                <g key={`laco-${sigla}`} className="carto-aresta">
                  ...
                  <path d={d} fill="none" stroke="transparent" strokeWidth={14}>
                    <title>{`${sigla}: ${n} produções em coautoria que não saem do programa`}</title>
                  </path>
                </g>
              );
            })}
      ...
      <div className="legenda">
        ...
        <span className="legenda-item">
          Espessura = produções em coautoria · barra à esquerda do bloco = volume nos tipos selecionados
        </span>
      </div>

      <Nota titulo="A rede, programa a programa" rotulo="Como ler">
        <h3>A ligação é medida em produções</h3>
        <p>
          Coautorias de um programa que incluem alguém que também atua no outro. Não se trata de "produção assinada
          por dois programas": isso não existe na base, porque cada produção pertence a exatamente um programa, e o
          mesmo trabalho reportado por dois entra como dois registros. O laço roxo de volta ao próprio bloco é a
          coautoria que <em>não sai</em> do programa.
          {parciais > 0 && (
            <>
              {" "}
              Nesta seleção, <strong>{parciais}</strong> combinação(ões) de par × tipo ficaram abaixo do limiar de 5
              pessoas e não entram na soma: as linhas mostradas são um piso, e não o total.
            </>
          )}
        </p>
        <h3>Posições esquemáticas</h3>
        <p>
          Os blocos não seguem a geografia: a grade preserva a ordem norte-sul e oeste-leste. Arraste um bloco para
          desembaraçar a parte que você estiver olhando e use o afastamento para abrir a grade inteira; nada disso
          altera os dados. A posição real de cada programa está no mapa-localizador.
        </p>
      </Nota>
