 ...
 * UMAP 3D puro, não o híbrido 2.5D nem o topográfico-geográfico). Ocupa o
 * mesmo contêiner medido do canvas 2D (`largura` × `altura`), para não saltar o
 * layout da página ao trocar de dimensão.
 *
...
/** Quanto se afastam os pontos uns dos outros (o cubo de referência passa de ~10 a ~13,5 de lado)
 * sem mexer no tamanho das esferas: mais ar entre elas. A câmera recua só `RECUO_CAMERA` (< a
 * expansão) para a nuvem continuar cabendo no quadro — o que faz as esferas parecerem um pouco
 * menores e o espaço entre elas, maior. Ajuste aqui se quiser mais ou menos. */
const ESPACAMENTO_3D = 1.35;
const RECUO_CAMERA = 1.3;
...
interface Props {
  ...
  /** Tamanho medido do contêiner (o mesmo do canvas 2D). */
  largura: number;
  altura: number;
  /** `cliques` é o `detail` do clique: 2 no segundo clique de um duplo clique. */
  onSelecionar: (p: AtlasProjeto, cliques: number) => void;
...
export default function Atlas3D({
  ...
  // A cena monta uma vez; o tamanho do quadro é lido de fora a cada mudança.
  const tamanhoRef = useRef({ largura, altura });
  ...
  useEffect(() => {
    ...
    if (!container) return;
    const { largura: w0, altura: h0 } = tamanhoRef.current;

    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera(50, w0 / h0, 0.1, 100);
    camera.position.set(0, 0, 13 * RECUO_CAMERA);
  ...
  // Acompanha o contêiner: a câmera e o renderer seguem o tamanho medido,
  // sem reconstruir a cena (a rotação que a pessoa já fez fica).
  useEffect(() => {
    const st = stateRef.current;
    if (!st) return;
    st.camera.aspect = largura / altura;
    st.camera.updateProjectionMatrix();
    st.renderer.setSize(largura, altura);
  }, [largura, altura]);
  ...
  // pro mesmo cubo de referência (~10 × `ESPACAMENTO_3D` de lado) independente da escala
  // natural de cada método, senão a câmera fixa (z = 13 × `RECUO_CAMERA`) ficaria longe
  ...
  useEffect(() => {
    ...
    const escala = (10 * ESPACAMENTO_3D) / maiorFaixa;
  ...
  return <div ref={mountRef} style={{ width: largura, height: altura, touchAction: "none" }} />;
}
